...
export default function SubscriptionsPage() {
			...
			const subsWithUsers = await Promise.all(
				subs.map(async (sub) => {
					try {
						...
						return {
							...sub,
							user: userData || { email: 'Unknown', name: 'Unknown User' },
						};
					} catch {
						return {
							...sub,
	...
	const getActiveCount = () => {
		return subscriptions.filter((s) => {
			const now = new Date();
			return (
				new Date(s.dateTheSubscriptionStarts) <= now &&
				new Date(s.dateTheSubscriptionEnd) >= now
			);
		}).length;
	};
	...
	const getExpiringSoon = () => {
		return subscriptions.filter((s) => {
			const end = new Date(s.dateTheSubscriptionEnd);
			const now = new Date();
										...
										<tr
											...
											<td className='px-6 py-4'>
												<span
													...
													<Icon
														icon={
															isActive ? 'mdi:check-circle' : 'mdi:close-circle'
														}
														width='16'
													/>
